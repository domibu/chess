...
void printmoves_c( move *m_l, int depth)
{
	/*int count = 0;
	move *it;
	for ( it = depth; it; it--;)
...
int negamax( board *pos, int alpha, int beta, int color, int depth)
{
	int best, val;
	move *list, *it;
	
	if ( !depth ) return color * eval( pos);
	
	best = -WIN;
	list = generate_moves( *pos);
	
	for (it = list; it; it = it->next )
    	{

        	do_move(pos, it);
	        val = -negamax( pos, -beta, -alpha, -color, depth - 1);
        	undo_move(pos, it);
...
int rootnegamax( board *pos, move *fst_pick, int alpha, int beta, int color, int depth)
{
	int best, val;
	move *list, *it;
...
int n = 0, score, color;
board cb;
move pick, *list, *domove = NULL, *temp;
char FEN[100];
