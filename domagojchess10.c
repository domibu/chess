...
{
	int best, val, old_alpha = alpha;
	move *list, *it, *TTmove = NULL, *pick = NULL;
	line nline;
	TTentry *entry;
	...
	for (it = list; it; it = it->next )
		...
		if ( val >= beta)
		{
			...
			TTstore( pos->zobrist, pick, depth, val, 2);
	...
	if (best <= old_alpha)
	{
			...
			TTstore( pos->zobrist, pick, depth, best, 3);
	}
...
char starting_position[100] = "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 0";//pos 2 ply 6 8.031.647.685
...
int n = 0, score, color;
board cb;
move *fst_pick, *list, *domove = NULL, *temp;
