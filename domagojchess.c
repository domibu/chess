	...
	while (__builtin_popcountll(at_r) )
	{
		...
		//while (__builtin_popcountll(ppb))
		{
					...
					if ( arg.w.P & ppr)
			{
				...
				mpp = __builtin_ffsll( mpr & ((((0x000000000000FF00 & ppr) << 8) & ~arg.b.pieces) << 8) & ~arg.b.pieces);// za dva
	/*printf("pawn_pinned\n");
...
//PITANJE DA LI PAWN ADVANCE JE QUIET ILI CAPTURE
				...
				//else 
				{
					mpp = __builtin_ffsll( mpr & (ppr << 8) & ~arg.b.pieces);// za  jedan 
					if (mpp)
					{
						//promotion nije kad je en'passan
						//capture = 1LL << 6;
...
void printmoves(move *m_l)
{
	...
	while ( m_l)
	{
		...
		printf("m_l: stm %d piece %d from %d dest %d capture %d check %d enp %d pawn_adv %d\n", stm, piece_type, from, dest,capture,check,enp,pawn_adv);
		printBits(1, &d_pa);
